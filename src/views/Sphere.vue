  ...
  <dv-border-box-1 class="container" :color="dvColor" v-loading="isLoading"
    ...
    <div :color="dvColor" class="top-div">
      <div class="name-div" :style="{ backgroundColor: sysBackgroundColor }">
        <dv-decoration-1 class="name-dv" :color="dvColor" />
        <dv-decoration-11 class="sys-name" :color="dvColor">
          疫情可视化
        </dv-decoration-11>
        <dv-decoration-1 :reverse="true" class="name-dv" :color="dvColor" />
      </div>
    ...
    <dv-border-box-4 :reverse="true" :color="dvColor" class="sphereDataDiv">
      ...
      <dv-decoration-3 :color="dvColor" style="width:80%;height:20px;margin: -20px 0px 0px 10%" />
    ...
    <!--设置icon按钮-->
    <div class="set-div">
      <el-icon color="white" :size="40" @click="isDrawer = true">
        <Setting />
      </el-icon>
    </div>
...
let version: any = ref(PK.version),//系统版本号
  ...
  sysBackgroundColor: any = 'rgb(255, 255, 255, .2)',//系统背景主题色
...
function destroyScene() {
  ...
  renderer.dispose();//撤销当前渲染器窗口，并释放资源
  scene.clear();//清除场景
  scene = null;
  camera = null;
  orbitControls = null;
...
function structureData(COVID19Data: any) {
  ...
  sphereData.value = worldlist; //把全球列表数据赋值给球体数据
  othertotal.value = allData.value.othertotal;
  //国内省份数据类型转换
  allData.value.list.forEach((l: any) => {
    l.value = Number(l.value);
    l.econNum = Number(l.econNum);//现存确诊
    l.deathNum = Number(l.deathNum);//死亡
    l.cureNum = Number(l.cureNum);//治愈
    l.asymptomNum = Number(l.asymptomNum);//较昨日新增数
    l.jwsrNum = Number(l.jwsrNum);//境外输入数
    //市/区数据类型转换
    if (l.city.length !== 0) {
...
function init(data: any) {
  ...
  let width = dom.clientWidth;//返回元素可见宽度
  let height = dom.clientHeight;
  scene = new THREE.Scene(); //场景场景
  camera = new THREE.PerspectiveCamera(45, width / height, 1, 1000); //创建透视相机(视场、长宽比、近面、远面)
  camera.position.set(0, 50, 250); //设置相机再三维坐标中的位置
  camera.lookAt(0, 0, 0);//设置相机拍摄时指向的方向
  //创建渲染器
  ...
  createOrbitControls();//创建鼠标控制
  render();//调用渲染
  window.addEventListener('resize', onWindowResize, false);//只要窗口大小发生像素变化就会触发
};
...
function createUniverse() {
  let universeGeometry = new THREE.SphereGeometry(500, 100, 100);//几何体 半径，经纬度上的分段数，段数越多球体表面越光滑
  let universeMaterial = new THREE.MeshLambertMaterial({
    //高光材质
    map: new THREE.TextureLoader().load(universeImg),//map必须调用的是一个函数而不是变量
    side: THREE.DoubleSide, //双面显示 默认情况下是frontSide 还有BackSide
  });
  //宇宙网格  tips：几何体是不能被渲染的，只有几何体和材质结合成
...
function createLight() {
  let lightColor = new THREE.Color(0xffffff);//创建白色
...
async function createWBSphere(sphereType: any) {
  ...
  earthMesh.name = "地球";
  await earthGroup.add(earthMesh); //等待异步任务执行完成，将地球网格添加到地球组中
};
...
async function createSpotSphere() {
  ...
  let globeInnerMaterial = new THREE.MeshBasicMaterial({
    ...
    transparent: true,//透明
    opacity: .4,//不透明度
    fog: new THREE.Fog(0x050505, 2000, 3500),//雾化属性：设置周围雾的颜色，受雾影响最近距离，最远距离
...
function createSpot() {
  ...
  img.onload = () => {
    ...
    let o = null; //数组处理时的计数
    for (o = 0; o < canData.data.length; o += 4) {//一维数组，rgba顺序的数据的长度
    ...
    let positionVal = new THREE.BufferAttribute(l, 3); //设置缓冲区属性值，三个为一组，作为一个顶点的xyz坐标
    globeCloudBufferGeometry.setAttribute("position", positionVal); //给缓冲几何体添加位置属性
    let globeCloudMaterial = new THREE.PointsMaterial({//新建粒子材质
      color: new THREE.Color(dvColor.value[1]),//指定粒子颜色
      fog: true,
      size: 1.0,
      transparent: false,//是否透明
    });//球面斑点材质
    let d = new Float32Array(3 * globeCloudVerticesArray.length), c = [];//创建定点数组长度
    for (o = 0; o < globeCloudVerticesArray.length; o++) {
...
//经纬度坐标变换（传入e:纬度、a经度、t球半径、o球额外距离）得到每个点的坐标
function latLongToVector3(e: any, a: any, t: any, o: any) {
  let r = (e * Math.PI) / 180,//将纬度度数转换为弧度
    i = ((a - 180) * Math.PI) / 180,//经度弧度
...
function createVirus(data: any, earthSize: any) {
  ...
  list.forEach((e: { value: number; color: any; position: any[]; }) => {
    ...
    if (e.position) {
      let virusMaterial = new THREE.SpriteMaterial({//由于需要使用sprite作为标签，所以必须使用SM作为材质，本质上跟PM没有区别
        color: e.color,
...
function createRings() {
  createEquatorSolidRing(earthSize + 20);//创建赤道实线环,靠近球体最内环
...
function createEquatorSolidRing(r: any) {
  //创建里层的环
  let ringGeometry = new THREE.RingGeometry(r - 2, r + 2, 100);//内半径，外半径，分段数
  let ringMaterial = new THREE.MeshBasicMaterial({
    color: dvColor.value[0],
    opacity: .3,
    side: THREE.DoubleSide,//双面
    fog: true,//雾景
    depthWrite: false,//不要使用深度测试去深度缓冲区中写入
    blending: THREE.AdditiveBlending,//控制纹理融合叠加方式：将像素颜色添加到已绘制完的像素颜色上
  });
  let ringMesh = new THREE.Mesh(ringGeometry, ringMaterial);//创建出的环是平行于屏幕的，
  ringMesh.rotation.x = 90 * Math.PI / 180;//So需要X轴旋转90度
